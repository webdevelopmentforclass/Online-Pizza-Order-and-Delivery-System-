...
public class Bill2 extends JFrame implements ActionListener
{
		...
		Bill2(int ci)
				{
					...
					int i = 0,j = 0,x=0;
					pstmt2=con.prepareStatement("select order_name,quantity,bill,total_amount from Pizza_Hut where cust_id=?");
					pstmt2.setInt(1,c_id);
						...
						pstmt3=con.prepareStatement("select sum(total_amount) from Pizza_Hut where cust_id=?");
					pstmt3.setInt(1,c_id);
						...
						pstmt4=con.prepareStatement("select sum(total_amount)*0.12 from Pizza_Hut where cust_id=?");
					pstmt4.setInt(1,c_id);
							...
							pstmt7=con.prepareStatement("insert into  Pizza_Hut_Bill (cust_id,paid_amount) values(?,?) ");
			 	...
			 	{				
					JOptionPane.showMessageDialog(null,"Your order is saved...Thank you...!!!!");
						...
						if(ae.getSource()==b2)
							...
							pstmt6 =con.prepareStatement("delete from Pizza_Hut where cust_id=? ");
