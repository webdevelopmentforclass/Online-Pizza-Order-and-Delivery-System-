...
public class Bill4 extends JFrame implements ActionListener
{
		...
		Bill4(int ci)
				{
					...
					int i = 0,j = 0,x=0;
					pstmt2=con.prepareStatement("select order_name,quantity,bill,total_amount from Dominose where cust_id=?");
					pstmt2.setInt(1,c_id);
						...
						pstmt3=con.prepareStatement("select sum(total_amount) from Dominose where cust_id=?");
					pstmt3.setInt(1,c_id);
						...
						pstmt4=con.prepareStatement("select sum(total_amount)*0.12 from Dominose where cust_id=?");
					pstmt4.setInt(1,c_id);
							...
							l17.setText(Double.toString(d));
								...
								pstmt7=con.prepareStatement("insert into  Dominose_Bill (cust_id,paid_amount) values(?,?) ");
				...
				if(ae.getSource()==b1)
			 	{		
						...
						if(ae.getSource()==b2)
							...
							pstmt6 =con.prepareStatement("delete from Dominose where cust_id=? ");
